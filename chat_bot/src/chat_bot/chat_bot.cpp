...
ChatBot::ChatBot() : rclcpp::Node("chat_bot")
{
   ...
   // Iniciando um array de regexs
   this->regexs_ = std::vector<std::regex>({grupo1, grupo2, grupo3});

   // <---- Iniciando a definição do mapping de ações para cada comando ---->
   ...
   // <---- Finalizando a definição do mapping de ações para cada comando ---->
...
{
   // Loop para esperar o comando do usuário
   while (rclcpp::ok())
   {
      std::string command;
